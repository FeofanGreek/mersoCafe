...
class _profileScreenState extends State<profileView> {
  ...
  //таймер для повторной СМС


  //конец таймера для повторной СМС
  ...
  void initState() {
    ...
    _controllerPartnerCode = TextEditingController(text: partnerCode);
  ...
  Widget build(BuildContext context) {
    ...
    dellAccount() async{
      ...
      var profile = jsonEncode(<String, dynamic>{"name": "","phone": "", "addres":"", "partnercode":""});
...
readProfile() async {
  ...
  } catch (error) { print('ошибка $error');}
  if(Uname.length >1){if(Uphone.length > 5){if(Uaddress.length >5){ProfileComplite = true;clientPhoneChecked = true;
  textMainScreen = '    Κάθε ένα από τα πιάτα μας είναι μια δημιουργία στα πρόθυρα της τέχνης ενός σεφ και ενός καλλιτέχνη.\n Και κάθε ένας από τους επισκέπτες μας είναι επισκέπτης της έκθεσης τέχνης της τέχνης του σεφ!\n Καλή όρεξη!';
...
updateProfile() async {
  ...
  final Directory directory = await getApplicationDocumentsDirectory();
  final File file = File('${directory.path}/profile.txt');
  var profile = jsonEncode(<String, dynamic>{"name": "$Uname","phone": "$Uphone", "addres":"$Uaddress", /*"partnercode":"$partnerCode"*/});
  await file.writeAsString(profile);
